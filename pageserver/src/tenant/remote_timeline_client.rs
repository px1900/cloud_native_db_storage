...
impl RemoteTimelineClient {
    ...
    ///
    /// XI: Will do upload_queue creation during this function. But the status for upload_queue
    ///     is unintialized. Should call init_upload_queue out of this function before any uploading
    pub fn new(
    ...
    ///
    /// XI: With a pre-existing remote timeline, it will initialize the upload_queue
    ///     with its index_part. And recording the total remote layer file size into metrics.
    ...
    ///
    /// XI: Initialize an upload_queue without may existing layer files in the remote storage
    ...
    pub fn init_upload_queue_stopped_to_continue_deletion(
        ...
        //XI: The delete_at is a datetime type
        ...
        //XI: Initialize the upload_queue with index_part
        {
            let mut upload_queue = self.upload_queue.lock().unwrap();
            upload_queue.initialize_with_current_remote_index_part(index_part)?;
            self.update_remote_physical_size_gauge(Some(index_part));
        ...
        //XI: Convert the Initialized state to Stopped state.
        //     In this function, it will cancel the pending tasks and let in progress tasks
        //     keep going. However, because this is a newly initialized UploadQueue, it shouldn't
        //     have any pending or in progress tasks. So, I think this function will only do
        //     the conversion from Initialized to Stopped.
    ...
    //XI: Get remote_consistent_lsn_projected from the UploadQueueInitialized variable
    ...
    //XI: Iterate all layer files metadata, and calculate the sum of all layer files size. Then put
    //     this statistic data into metrics.
    ...
    pub async fn download_index_file(
        ...
        //XI: Call download function and record metrics
        ...
        //XI: Tell whether this is a deleted timeline? or a deleted index?
        //    If this index part is recorded as deleted, why does it exist in
        //    remote storage?
        //    It seems that after a timeline is deleted, the index part will
        //    not be deleted, because the following download_layer_file doesn't
        //    have this kind of deleted issues.
    ...
    ///
    /// XI: Update the in-memory upload_queue.latest_metadata first, which
    ///    will be ahead of remote storage's status. After that, schedule
    ///    a upload task, which execute until all previous layer files
    ///    upload were launched.
    ...
    ///
    /// XI: Insert this upload operation to the queued list, and try to launch this task.
    ///     If the latest_files_changes_..._scheduled is 0, this means after last metadata upload
    ///     there is no file changes, we don't need to do metadata uploads because the metadata
    ///     should not change.
    ...
    ///
    /// XI: Initialize a UploadOp::UploadMetadata operation, and insert this operation to
    ///     queued_operations list. This operation won't be sheduled until all previous operations
    ///     in this queued_operations list have been completed.
    ///     The function launche_queued_tasks will try to launch as many tasks as possible, until
    ///     the front operation need all previous operations have completed.
    fn schedule_index_upload(
        ...
        //XI: Index part includes layer_metadata, metadata and consistent lsn
    ...
    ///
    /// XI: Insert Op to queued_operations and update corresponding statistics
    ///     Then call launch_queued_tasks to schedule as many tasks as possible
    ...
    //XI: Update the latest_files hashmap and insert the operation to queued_operations
    ...
    /// successfully.
    ///
    /// XI: This function will firstly update the metadata, and insert the upload_index to the
    ///     queued_operations list. Then it will insert the to be deleted layers files deletion
    ///     operation to the queued_operations list.
    pub fn schedule_layer_file_deletion(
        ...
        //XI: Delete these files from metadata, and insert the layers to dangling_file list.
        //    And insert the upload_index operation to queued_operations list.
        //    The with_generations are the layers that actually exist in the metadata. (Note
        //     that it is possible that some item existing in $names argument are actually not
        //     existing in the metadata.)
        let with_generations =
            self.schedule_unlinking_of_layers_from_index_part0(upload_queue, names.iter().cloned());

        //XI: Remove the with_generations items from dangling_files and insert the deletion
        //      operations to queued_operations list.
    ...
    /// is invoked on them.
    ///
    /// XI: The gc_layers is a list of layers files that need to be gc.
    ///     However, this function will only update the metadata, and schedule an upload_index task.
    ///     The gc layer files will be inserted into dangling_files and no deletion tasks will be
    ///     scheduled for these remote layers.
    ...
    ///
    /// XI: This function works to delete layers from metadata.
    ///     This function will be called when there are some layers to be deleted.
    ///     Before the layers are actually deleted from remote storage, the metadata will
    ///     change first.
    ///     For this function, the $names is the names of layer files to be deleted.
    ///     This function will first update the local meta-data by deleting target layer file items.
    ///     After that, it will add these layer names to dangling_files list, which means these layers
    ///     should be deleted, but haven't been deleted yet.
    ///     Then it will schedule an index update task.
        ...
        I: IntoIterator<Item = LayerFileName>,
    {
        ...
        //XI: Delete the metadata items relating with the $names (to be deleted layer names)
        //  and the with_generations is a list contains the layer names that actually exist in the
        //  metadata hashmap.
    ...
    ///
    /// XI: Move the to be deleted layer files from dangling_file list to queued_operations list.
    ///     And try to launch tasks for these deletion operations.
    ...
    //XI: Delete the with_generation items from dangling_files.
    //    And insert the deletion operations to the queued_operations list.
    fn schedule_deletion_of_unlinked0(
        ...
        //XI: I guess this for loop aims complain when this function was called to delete a file with
        //  an exact generation, but it found there are some other tasks update the to be deleted file's
        //  generation. Don't know when will this logic work.
        #[cfg(feature = "testing")]
        for (name, gen) in &with_generations {
    ...
    ///
    /// XI: 1. Insert the new layers uploading operations to queued_operations list.
    ///     2. Delete the old layers from index metadata, and insert the upload_index operation
    ///         to the queued_operations list.
    ///     Note that: the old files layers will only be inserted into dangling list, and won't
    ///     schedule actual deletion tasks for them.
    ...
    ///
    /// XI: Insert a Barrier to queued_operations. Since the program will launch task for operations
    ///     from front to end, and this Barrier operation won't be launched until all previous in
    ///     progress tasks have completed, this barrier will actually wait for all previously launched
    ///     tasks to complete.
    ...
    //XI: Insert Barrier to queued_operations and try to launch tasks.
    fn schedule_barrier(
        self: &Arc<Self>,
        upload_queue: &mut UploadQueueInitialized,
    ...
    ///
    /// XI: Overall, this function will upload an IndexPart, which upload_queue status is STOPPED,
    ///     to remote. This function will only be executed by one thread, which is controlled by the
    ///     SetDeletedFlagProgress. At this function start, the FlagProgress should be ::NotRunning,
    ///     and this function will set it to InProgress and start the Uploading work. If Uploading work
    ///     failed, the ScopeGuard will automatically called undo_delete_at to set the status back to
    ///     ::NotRunning. However, if this uploading work is successful, it will cancel the undo_delete_at
    ///     and set status as the SetDeletedFlagProgress::Successful.
    #[instrument(skip_all)]
    pub(crate) async fn persist_index_part_with_deleted_flag(
        self: &Arc<Self>,
    ) -> Result<(), PersistIndexPartWithDeletedFlagError> {
        //XI: Get the uploaded_queue, and this upload_queue should stopped, and should not be called
        //    to be deleted. Which means the upload_queue should be UploadQueue:Stopped, and its
        //    deleted_at should be ::NotRunning
        //    Then set this upload_queue's delete at as current time.
        //    After that initialize an index part for this upload_queue
        let index_part_with_deleted_at = {
            ...
            //XI: For the upload_queue's stopped, there are three status:
            //  1. NotRunning - 2. InProgress - 3. Successful
            //  For the 2 and 3, this queue has already be called to stop by other progresses.
            //  Only when the status is NotRunning, we will keep going without reporting error.
        ...
        //XI: prepare a closure to undo the delete_at
        //   Define the closure with ScopeGuard, when exit the scope, this closure will be called automatically.
        //   So, when the following uploading work failed and returned, this closure will set the delete_at back to NotRunning
        ...
        //XI: Do the upload::upload_index_part with maximum two retries.
        backoff::retry(
            || {
        ...
        // XI: If we reached here, it means we successfully uploaded the index part. So, we don't
        //     need to call undo_deleted_at(). We use ScopeGuard::into_inner() to retrieve the ownership
        //     of the undo_deleted_at() closure, and set the delete_at status from InPorgress to Success.
        //
        // XI: This into_inner() function will retrieve the ownership from ScopeGuard back to this function,
        //     and thus cancel the auto-running closure.
        ScopeGuard::into_inner(undo_deleted_at);
        // XI: Do the status shift from NotRunning to Successful.
        {
            let mut locked = self.upload_queue.lock().unwrap();
    ...
    ///
    /// XI: It will delete the layers existing in self.upload_queue.latest_files.
    ///     After that, it will fetch the remaining layer names from the remote storage timeline.
    ///     Then, delete all of them apart from the latest index file. After that, delete latest index.
    pub(crate) async fn delete_all(self: &Arc<Self>) -> anyhow::Result<()> {
        debug_assert_current_span_has_tenant_and_timeline_id();

        // XI: For the stopped UploadQueue, collect the inner latest_files Hashmap items.
        //     For each layer file in latest_files, parse it into RemotePath.
        ...
        // XI: Check the remaining files for this storage timeline.
        ...
        // XI: It seems it try to fetch the index file with the largest generation.
        ...
        //XI: Other page layers and stale index layers
    ...
    ///
    /// XI: This function will retrieve operations from queued_operations vector from front to end.
    ///     It will try to simultanuously launch as many tasks as possible. For the UploadOp::UploadLayer
    ///     operations, it can always be scheduled immediately. For the UploadOp::UploadMetaData or
    ///     UploadOp::Barrier operations, it can't be scheduled until all inprogress tasks are completed.
    ///     For the deletion, it needs to wait until all in progress tasks are all for deletion.
    ///     If the front operation can be scheduled immediately, this function will return.
    ///     This function will insert the operations that can be called immediately to inprogress_tasks
    ///     list and call perform_upload_task to do real upload/download work.
    ...
    /// queue.
    ///
    /// XI: For the uploading metadata or layer files, just call remote_timeline_client/upload.rs
    ///     corresponding functions. For the deletion operations, insert this operation to queued list.
    ///     For the UploadMetadata, there is something about $future_layer, don't understand what
    ///     does that mean.
    ...
    //XI: For each type of UploadOp, return its FileKind, OpKind and file size.
    ...
    ///
    /// XI: This function will initialize a UploadQueue::Stop and use this Stop value to swap
    ///     the self.upload_queue. After that, it will let the in progress tasks keep going and
    ///     let pending tasks cancelled both by calling drop() function. (Don't know how does it
    ///     internally work)
        ...
        match &mut *guard {
            ...
            UploadQueue::Initialized(initialized) => {
                ...
                let qi = {
                    ...
                    // XI: Clone a new UploadQueueInitialized and replace the self.upload_queue
                    //    with UploadQueueStopped
                    ...
                    //XI: Swap the original self.upload_queue with the new UploadQueue:Stop
                    ...
                    //XI: Now the $qi is the original UploadQueueInitialized
