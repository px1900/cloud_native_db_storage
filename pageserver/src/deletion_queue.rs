...
//XI: This file accumulates deletion operations in the local disk, and do remote deletions
//    in batch. The reason is that the deletion has lower priority. If the database aims to
//    delete one layer file, it only needs to update the metadata to exclude this layer file,
//    and upload this updated metadata to remote storage. After that, the database will think
//    this file has been deleted. The deferred deletion can avoid competing network resources
//    with time sensitive uploading operations. The worst case for deferred deletion is that
//    the deletion operation will be lost, which at most cause remote storage leak.
